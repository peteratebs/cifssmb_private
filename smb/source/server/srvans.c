  ...
  PRTSMB_HEADER pHeader, PRTSMB_SESSION_SETUP_AND_X_R pSessionR)
{
  ...
  BBOOL extended_security_is_init_response=FALSE;
  int spnego_blob_size;
  byte *spnego_blob;
  byte spnego_blob_buffer[512];

  s = buf;
  extended_security_is_init_response = (pHeader->status == SMB_NT_STATUS_MORE_PROCESSING_REQUIRED);
  ...
  // guest_logon is phase one as action otherwise it is 0
  if (extended_security_is_init_response)
  { // Extended security final response this is reserved
  ...
  if (extended_security_is_init_response)
     spnego_blob_size=spnego_encode_ntlm2_type2_response_packet(spnego_blob_buffer, sizeof(spnego_blob_buffer),glencryptionKey);
  else if (pHeader->status != SMB_NT_STATUS_LOGON_FAILURE)
     spnego_blob_size=spnego_encode_ntlm2_type3_response_packet(spnego_blob_buffer, sizeof(spnego_blob_buffer));
  RTSMB_PACK_WORD (spnego_blob_size); // Blob length
  pbytecount = buf;
  RTSMB_PACK_WORD (0);  /* byte count */

  bs = buf;  /* measure start of data section */

  if (pHeader->status != SMB_NT_STATUS_LOGON_FAILURE)
  {
    RTSMB_PACK_ITEM (spnego_blob_buffer, spnego_blob_size);
  }
  ...
  PRTSMB_HEADER pHeader, PRTSMB_SESSION_SETUP_AND_X_R pSessionR)
{
  ...
  // HEREHERE guest_logon is phase one as action otherwise it is 0
  RTSMB_PACK_WORD ((word) ((pSessionR->guest_logon) ? 1 : 0));
