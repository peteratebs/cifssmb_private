...
BBOOL ProcSMB1NegotiateProtocol (PSMB_SESSIONCTX pCtx, SMB_DIALECT_T dialect, int priority, int bestEntry, PRTSMB_HEADER pInHdr, PFVOID pInBuf, PRTSMB_HEADER pOutHdr, PFVOID pOutBuf)
{
    int i;
    int authmode = pCtx->accessMode;
    if ((dialect <= PC_NETWORK) || priority < 1)
    {
    ...
    case SMB_COM_WRITE_RAW:
    {
        ...
        else
        {
            if (poutCliHdr->status == 0)
            {
                ...
                SMBS_Setsession_state(pSctx, WRITING_RAW);
