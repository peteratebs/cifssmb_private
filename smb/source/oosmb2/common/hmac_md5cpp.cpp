...
#include "hmac_md5.hpp" // "md5rsa.hpp"
...
#define MD5_FINAL(key, context)        MD5Final(context);                   \
                                       memcpy(key, (context)->digest, 16)
#define MEMSET(mem, val, size)		   memset(mem, val, (unsigned int)size)
#define MEMCPY(to, from ,len)		   memcpy(to, from, (unsigned int)len)
/**
