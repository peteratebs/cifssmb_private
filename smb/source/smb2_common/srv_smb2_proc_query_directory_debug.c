...
// See if we have a match for this file id
{
  ...
  int _sid;
  *psid = 0;
rtsmb_dump_bytes("SEACRH FOR FID ",  pFileId, fidsize,  DUMPBIN);
  for (_sid = 0; _sid < prtsmb_srv_ctx->max_searches_per_uid; _sid++)
  {
    if (user->searches[_sid].inUse)
    {
      rtsmb_dump_bytes("MATCH AGAINST :",  user->searches[_sid].FileId, fidsize,  DUMPBIN);
    }
...
#if (0)
rtsmb_dump_bytes("SEACRH FOR FID ",  pFileId, sizeof(command.FileId),  DUMPBIN);
    {
        int _sid;
        sid = 0;
        for (_sid = 0; _sid < prtsmb_srv_ctx->max_searches_per_uid; _sid++)
        {
if (user->searches[_sid].inUse)
rtsmb_dump_bytes("MATCH AGAINST :",  user->searches[_sid].FileId, sizeof(command.FileId),  DUMPBIN);
...
printf("Search not found !!\n");
    	for (sid = 0; sid < prtsmb_srv_ctx->max_searches_per_uid; sid++)
            ...
            {
printf("Sid grabbed free handle # is %x\n", sid);
    			break;
            }
...
printf("Sid # is %x\n", sid);
rtsmb_dump_bytes("Saving search INPUT FILID",  pFileId, sizeof(command.FileId),  DUMPBIN);
{
int check_sid;
...
printf("Saving pattern this size: # %d\n", strsize);
rtsmb_dump_bytes("Save pattern", file_name, strsize, DUMPUNICODE);
     }
...
//     if (pStream->compound_output_index == 0 || (command.Flags & (SMB2_RESTART_SCANS|SMB2_REOPEN)))
    ...
    {
rtsmb_dump_bytes("!!! Gfirst on", user->searches[sid].name, 2*rtsmb_len(user->searches[sid].name), DUMPUNICODE);
