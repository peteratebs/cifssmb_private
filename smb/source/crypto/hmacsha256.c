...
void calculate_smb2_signing_key(void *signing_key, void *data, size_t data_len, unsigned char *result)
{
  ...
  uint8_t *bdata = (uint8_t *) data;
        ...
        memset(&m, 0, sizeof(m));
        hmac_sha256_init(signing_key, 16, &m);
        hmac_sha256_update(data,data_len,&m);
#if (0)
        hmac_sha256_update(&bdata[0],64,&m);
        hmac_sha256_update(&bdata[64],8,&m);
        hmac_sha256_update(&bdata[72],29,&m);
#endif
