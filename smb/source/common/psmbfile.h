...
typedef BBOOL (RTSMB_FAR *RTSMB_FS_DIRENTCOUNTFN)(char RTSMB_FAR * name,int max_count);
...
typedef BBOOL (RTSMB_FAR *RTSMB_FS_WDIRENTCOUNTFN)(unsigned short  RTSMB_FAR * name);
...
typedef struct smbfileapi
{
    ...
    RTSMB_FS_DIRENTCOUNTFN fs_direntcount;
...
struct smbfstat
{
	...
	unsigned char  unique_fileid[SMB_UNIQUE_FILEID_SIZE]; /* Passed back in Fileid field in SMB2  */
};
