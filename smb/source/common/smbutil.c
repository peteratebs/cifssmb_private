...
BBOOL gl_display_login_info; // Also in server context, this is so we can share with client & server
...
// PFCHAR password, PFBYTE serverChallenge, PFRTCHAR domainname, PFBYTE ntlm_response_blob, size_t ntlm_response_blob_length, PFRTCHAR name, PFCHAR output)
//
...
// Strip null ternmiations off strings if they were provided.
...
BYTE user_domain[512];
  ...
  if (gl_display_login_info) {rtsmb_dump_bytes("NTLMv2 owf nt hash: ", outowf, 16, DUMPBIN);}
  rtsmb_util_string_to_upper ((PFRTCHAR)user_name, CFG_RTSMB_USER_CODEPAGE);
  if (gl_display_login_info) {rtsmb_dump_bytes("User", user_name,user_name_size, DUMPASCII);}
  if (gl_display_login_info) {rtsmb_dump_bytes("Domain", domain_name,domain_name_size, DUMPASCII);}

  user_name_size = trim_right_null(user_name,user_name_size);
  if (user_name_size > 0)
    tc_memcpy(user_domain,user_name,user_name_size);
  ...
  if (gl_display_login_info) {rtsmb_dump_bytes("User_Domain", user_domain, user_name_size+domain_name_size, DUMPASCII);}
  ...
  if (gl_display_login_info) {rtsmb_dump_bytes("NTLMv2 enrypted key calculated output: ", kr, 16, DUMPBIN);}

   // Comes from  pStream->psmb2Session->pSmbCtx->encryptionKey, the encrypted key send by the client.
  if (gl_display_login_info) {rtsmb_dump_bytes("Security blob: ", security_blob, blob_size, DUMPBIN);}


  cli_util_encrypt_signing_key_ntlmv2 ((PFBYTE)encrypted_key, (PFBYTE)security_blob, (size_t) blob_size, (BYTE *) kr, (PFCHAR)encsignkey);

  if (gl_display_login_info) {rtsmb_dump_bytes("NTLMv2 key_ntlmv2 encsignkey: ", encsignkey, 16, DUMPBIN);}
...
// output from cli_util_encrypt_signing_key_ntlmv2  == CE 98 06 7A BA 98 03 80   AF 1C 6C 04 A9 95 87 38
  ...
  if (gl_display_login_info) {rtsmb_dump_bytes("SMBsesskeygen_ntv2: ", sess_key, 16, DUMPBIN);}
  // should be  0x63, 0xA2, 0x4F, 0xB3, 0x81, 0x6B, 0x85, 0x99, 0xEC, 0xBA, 0x0D, 0x04, 0xB0, 0x8A, 0xC7, 0xCC};          // Output of SMBsesskeygen_ntv2

  if (gl_display_login_info) {rtsmb_dump_bytes("session_key: ", session_key, session_key_size, DUMPBIN);}
  ...
  if (gl_display_login_info) {rtsmb_dump_bytes("signing_key_result: ", signing_key_result, 16, DUMPBIN);}
