...
#define SIGN_HMAC_SHA256  2                         // - Used for 2.002 and 2.100 generates 32 byte hash over entire message including Header and padding. Copy low 16 bytes into the keyfield
    ...
    void *   write_origin;                          // Points to the beginning of the buffer, the NBSS header.
    void *   saved_write_origin;                    // Original origin if the packet is beign encrypted
    void *   pOutBuf;                               // Current position in the output stream buffer.
